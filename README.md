...
- 课程订阅：每个用户独立选择关注的课程
- 统一管理事项：课程、作业和个人待办都可以纳入提醒体系
- 管理待完成作业（命令 / JSON 文件 / 周期性规则三种方式），每人独立标记完成
- 作业编号按用户独立分配并保持稳定；同一条公共作业在不同用户视角下编号可以不同
- 自动发送作业截止提醒（4 级递进提醒），按用户订阅独立发送
- 设置任意个人提醒（取快递、开会、吃药...），到点私信通知
- 用 `/agenda` 查看今日课程、待完成作业和待发送提醒的统一总览
- 查看今日课程和学期课表，为已订阅课程生成上课提醒
- 默认每日 08:00 推送个性化早报（今日课程 + 近期作业 + 今日提醒）
...
| 模块 | 能力 |
|------|------|
| 用户与权限 | root / 管理员 / 普通用户 / 待审核用户；新用户注册审批；root 可授予管理员 |
| 课程 | 解析教务系统 `course.txt`；按用户订阅课程；支持私人课程和公共课程；支持上课提醒 |
| 作业 | 手动添加、JSON 导入、周期规则生成；公共作业和私人作业隔离；完成状态按用户独立记录 |
| 提醒 | 一次性提醒、每日重复提醒、作业截止提醒、上课提醒、每日早报 |
| 事项总览 | `/agenda` 汇总今日课程、待完成作业、待发送提醒 |
| LLM | OpenAI 兼容接口；自然语言触发工具调用；支持公开问答和个人事项操作 |
...
#### 使用内置 NapCat Docker 配置

仓库提供了一个 NapCat Docker Compose 示例，适合本机长期运行：
...
如果从其他机器访问，把 `localhost` 换成宿主机 IP。进入 WebUI 后登录 QQ，并配置反向 WebSocket：
...
注意：当前 [`deploy/napcat/compose.yaml`](./deploy/napcat/compose.yaml) 里 `ACCOUNT` 是直接写在 `environment` 中的；如果要换登录 QQ 号，请修改 `ACCOUNT`。
...
| 角色 | 说明 | 权限 |
|------|------|------|
| `root` | `OWNER_QQ` 对应的用户，启动时自动创建 | 所有权限 + 查看所有用户 + 授予管理员 |
| `admin` | 管理员，由 root 授予 | 管理公共作业/公共课程 + 审批用户 + 普通用户全部权限 |
...
4. 用户收到审批通过后即可使用所有功能，建议先发送 `/agenda` 查看事项总览，再发送 `/subscribe` 查看可选课程并按需订阅
...
- `/list` 里看到的作业编号是“当前用户自己的显示编号”，不是全局数据库主键
- 同一条公共作业在不同用户的 `/list`、提醒正文、早报里可能显示为不同编号
- `/done <编号>`、`/delete <编号>`、快捷 `3` / `x3` 都按你自己当前看到的作业编号生效
- 完成状态始终按用户独立记录；你完成一条公共作业后，只会影响你自己的列表和提醒

### 事项总览

| 命令 | 说明 | 权限 | 别名 |
|------|------|------|------|
| `/agenda` | 查看统一事项总览（今日课程 + 待完成作业 + 待发送提醒） | 用户+ | `/待办` `/事项` `/总览` |
...
| `/dailyremind <HH:MM> <内容>` | 设置每日重复提醒 | 用户+ | `/每日提醒` |
| `/dailyreminds` | 查看每日重复提醒 | 用户+ | `/每日提醒列表` |
| `/canceldaily <编号>` | 取消每日重复提醒 | 用户+ | `/取消每日提醒` `/删除每日提醒` |
...
| `/briefing_time [HH:MM\|on\|off]` | 查看或修改每日早报时间和开关 | 用户+ | `/早报时间` |
| `/briefing_content [课程 作业 提醒]` | 查看或修改每日早报内容 | 用户+ | `/早报内容` |
| `/help` | 查看帮助 | 所有人 | `/帮助` |
...
| `/importcourses` | 按固定格式批量导入课程。管理员导入公共课程，普通用户导入私人课程 | 用户+ | `/导入课程` `/导入课表` |
...
`/importcourses` 的格式是“每行一门课”：`课程名 | 时间 | 教师 | 地点`。教师和地点可省略，也支持直接粘贴代码块。
...
LLM 会自动识别意图并执行对应操作。普通用户可以查看事项总览、管理自己的私人作业、私人课程和个人提醒；管理员可通过自然语言管理公共作业、公共课程，并审批普通用户；root 额外可以通过自然语言查看所有用户及其角色，并授予管理员。未审批用户只能进行公开问答。

- 用户仍然可以直接说自然语言时间，例如“下周五”“明天下午3点”
- LLM 会先把时间换算成标准格式再执行；如果时间不够明确，系统会先追问，不会直接写入模糊时间
- 自然语言里的“完成 #3”“删除 #3”同样使用你自己 `/list` 里看到的作业编号

当前 LLM 支持的主要动作：

- 查看事项总览、今日课程、作业列表、课程列表、已订阅课程
- 添加作业、完成作业、删除作业
- 添加一次性提醒、查看提醒、取消提醒
- 添加每日提醒、查看每日提醒、取消每日提醒
- 查看和修改早报时间、早报开关、早报内容
- 订阅课程、退订课程、订阅全部课程
- 添加课程、删除课程、开启/关闭上课提醒
- 管理员/root 查看待审核用户并审批；root 可查看全部用户并授予管理员

#### LLM 记忆与边界

当前 LLM 不是 RAG，也没有通用聊天历史记忆。

它每次回复时会临时读取并放入上下文的信息包括：
...
这些业务数据来自 SQLite 和课表文件，所以作业、提醒、课程订阅、早报设置等会持久保存。普通聊天内容不会作为对话历史长期保存；目前只有早报设置相关的少量追问状态会在内存里短时间保留，重启后消失。

当前没有实现：

- 文档切片、embedding、向量数据库或知识库检索
- 对 PDF/Word/图片课程资料的自动解析
- 通用多轮对话记忆，例如稳定理解“刚才那个”“改成三点”
- 直接修改已有作业截止时间；建议先删除旧作业再重新添加
...
作业提醒正文里的 `#编号` 也按接收该提醒的用户自己的作业编号生成。
...
也可发送 `/briefing` 手动查看。
用 `/briefing_time <HH:MM>` 修改推送时间，`/briefing_time off` 关闭，`/briefing_time on` 开启。
用 `/briefing_content 课程 作业 提醒` 设置早报内容；也可以只保留部分内容，例如 `/briefing_content 作业 提醒`。
...
| 每日早报 | 每分钟 | 扫描到点用户并推送个性化早报（默认时间 08:00，可配置） |
...
用户侧看到的作业编号也是每人独立的稳定编号，不同用户不保证一致。
...
- OpenAI 兼容 Chat Completions 接口；LLM 输出 JSON Action 后由本地工具函数执行

当前没有引入 LangChain、LlamaIndex、FAISS、Chroma、Milvus 等 RAG/向量检索框架。
...
这里说的“作业编号”是用户自己看到的显示编号；如果源文件里的作业被识别成新作业，用户侧编号也会重新分配。

### 为什么我和别人看到的作业编号不一样

这是预期行为。作业编号现在按用户独立分配，便于每个人直接用自己的 `/list` 编号执行 `/done` 和 `/delete`。同一条公共作业在你这里可能是 `#2`，在别人那里可能是 `#1`，但完成状态仍然各自独立。
