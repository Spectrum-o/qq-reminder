...
连接成功后，`OWNER_QQ` 私信发送 `/help` 查看所有命令。其他用户私聊 Bot 会自动注册为待审核状态，需管理员执行 `/approve` 审批后才能使用。
...
| `admin` | 管理员，由 root 授予 | 添加/删除作业 + 审批用户 + 所有用户权限 |
| `user` | 普通用户，由管理员/root 审批 | 查看/完成作业、设置提醒、订阅课程 |
| `pending` | 待审核，私聊 Bot 自动注册 | 无权限，等待审批 |

### 注册流程

1. 新用户私聊 Bot → 自动注册为 `pending`，收到"等待审核"提示
...
| 命令 | 说明 | 权限 | 别名 |
|------|------|------|------|
| `/add <课程> <时间> <描述>` | 添加课程作业 | 管理员+ | `/添加` |
| `/done <编号>` | 标记完成（仅自己） | 用户+ | `/完成` |
| `/delete <编号>` | 删除作业 | 管理员+ | `/del` `/删除` |
...
LLM 会自动识别意图并执行对应操作。管理员可通过自然语言添加/删除作业，普通用户只能标记完成和设置提醒。
...
订阅了对应课程的用户，上课前收到私信提醒（包含课程名、节次、时间、教师、教室）。
...
新用户私聊 Bot 会自动注册为 `pending` 状态，需要管理员执行 `/approve <QQ号>` 审批后才能使用。
